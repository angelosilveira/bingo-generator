          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 items-start">
            ...
            <form onSubmit={handleSubmit} className="space-y-5">
                ...
                <div className="space-y-4">
                  <div>
                    ...
                    <input type="text" value={form.premio} onChange={set('premio')}
                      ...
                      className="w-full px-4 py-2.5 border border-gray-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-[#1a3a6b]" required />
                  </div>
                  <div>
                    <label className="block text-sm font-medium text-gray-600 mb-1">Imagem do prêmio</label>
                    <div onClick={() => fileRef.current?.click()}
                      className="border-2 border-dashed border-gray-200 rounded-xl p-5 text-center cursor-pointer hover:border-[#1a3a6b] hover:bg-blue-50 transition-colors">
                      {imagePreview
                        ? <img src={imagePreview} alt="Preview" className="max-h-32 mx-auto rounded-lg object-contain" />
                        : <div className="flex flex-col items-center gap-2 text-gray-400">
                            <ImageIcon size={28} /><span className="text-sm">Clique para selecionar</span>
                    ...
                    {imagePreview && (
                      <button type="button" onClick={() => { setImageFile(null); setImagePreview(null) }}
                        className="text-xs text-red-500 mt-1 hover:underline">Remover imagem</button>
                    )}
                  </div>
                </div>
              </section>
              ...
              <button type="submit" disabled={submitting}
                className="w-full flex items-center justify-center gap-2 bg-[#1a3a6b] hover:bg-[#0f2347] text-white font-bold py-3.5 rounded-xl transition-colors disabled:opacity-60 text-base">
                {submitting
